...
## 	#this creates an actual array, not an iterator, so it won't work later:
## 	result = []
## 	clipped = hard_clip(generator, min, max)
## 	samples = normalize(clipped, min, max, -2**(width * 8 - 1), 2**(width * 8 - 1) - 1)
## 	for sample in samples:
## 		print sample
## 		temp = struct.pack(fmt, int(sample))
## 		result.append( temp )
...
## 	return result
	...
	#python list comprehension is an iterator by default:
